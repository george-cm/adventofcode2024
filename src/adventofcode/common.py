...
def create_day(day: int) -> None:
    ...
    new_day_test_file_name = f"{new_day_test_file_name_template.substitute(day=day_str)}"

    with day_test_file.open("r", encoding="utf-8") as f:
        new_day_test_file_content = f.read()
        new_day_test_file_content = Template(new_day_test_file_content).substitute(day=day_str)
